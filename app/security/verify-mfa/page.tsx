...
// Verificación MFA para una sesión ya registrada (aal1 → aal2). Pide el código
// del Authenticator del propietario.
  ...
  useEffect(() => {
    ...
    supabase.auth.mfa
      ...
      .then(({ data }) => {
        ...
        if (!totp) {
          router.replace("/security/setup-mfa");
          return;
        }
        setFactorId(totp.id);
        setStatus("ready");
      ...
      .catch(() => {
        setError("No se pudieron cargar los métodos de verificación.");
        setStatus("error");
      });
  }, [router]);
