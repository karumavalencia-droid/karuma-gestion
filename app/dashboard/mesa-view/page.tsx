...
export default function MesaViewPage() {
  ...
  const [wiGeneral, setWiGeneral]   = useState(false); // true = opened from top bar (sin mesa de partida)
  const [wiMesaIds, setWiMesaIds]   = useState<string[]>([]); // mesas del walk-in (varias para grupos grandes)
  ...
  function openWalkIn(m: MesaConEstado) {
    ...
    setWiNotas(""); setWiError(""); setWiOk(false); setWiGeneral(false); setWiMesaIds([m.id]);
  }
  ...
  function closeWalkIn() { setWiMesa(null); setWiOk(false); setWiError(""); setWiGeneral(false); setWiMesaIds([]); }
  ...
  // Primera franja del plano en la que la mesa quedará libre tras su ocupación actual.
  function horaTrasOcupacion(m: MesaConEstado): string | null {
    const occ = (m.agenda ?? []).find((r) => r.estado === "sentada" || r.estado === "walkin");
    if (!occ) return null;
    const [h, mm] = occ.hora.split(":").map(Number);
    let fin = h * 60 + mm + (occ.duracionMin ?? duracionReserva(occ.personas));
    if (occ.fecha === hoy()) {
      const now = new Date();
      fin = Math.max(fin, now.getHours() * 60 + now.getMinutes());
    }
    return slotsPlano(servicio).find((s) => {
  ...
  async function submitSwap(objetivo: ReservaLocal) {
    ...
    // Sincroniza con Supabase ANTES de dar el cambio por bueno: si el servidor
    // rechaza alguna de las dos (p. ej. la reserva cambió en otro dispositivo),
    // se revierte el intercambio local para no quedar desincronizados.
    for (const [r, nuevas] of [[swapReserva, mesasB], [objetivo, mesasA]] as const) {
      if (!r.origen) continue;
      let error = "";
      try {
        const response = await fetch("/api/reservas/actualizar", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            action: "cambiar-mesa",
        ...
        const json = await response.json() as { ok?: boolean; error?: string };
        if (!response.ok || !json.ok) error = json.error ?? "No se pudo sincronizar el intercambio.";
      } catch {
        error = "No se pudo sincronizar el intercambio.";
      }
      if (error) {
        intercambiarReservas(swapReserva.id, objetivo.id); // revierte el swap local
        setSwapError(`${error} (${r.nombre})`);
        reload();
        return;
      }
    }
  ...
  // Plazas de las mesas elegidas para el walk-in; el selector de mesas aparece
  // en modo general, cuando el grupo supera el límite de plazas seleccionadas
  // y mientras haya varias mesas (para poder quitar una elegida por error)
  const wiPlazas = wiMesaIds.reduce((t, id) => t + (mesasList.find((m) => m.id === id)?.capacidad ?? 0), 0);
  const wiShowPicker = wiGeneral || wiPersonas > wiPlazas || wiMesaIds.length > 1;
      ...
      <div className="mesa-page-inner mx-auto max-w-5xl">
        ...
        <div className="mesa-grid grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
          {mesas.map((m) => {
            const r = m.reserva;
            const agenda = m.agenda ?? [];
            // "Próx" = siguiente turno desde la franja mostrada (los pasados,
            // p. ej. un walk-in ya terminado, no cuentan como próximos).
            const futuras = m.status === "available" ? agenda.filter((a) => a.hora >= horaPlano) : [];
              ...
              <button key={m.id} onClick={() => handleMesaClick(m)}
                ...
                {m.status === "reserved" && r && !isTableBlockReservation(r) && (
                  <div className="mesa-card-detail mt-2 space-y-0.5">
                    ...
                    <div className="mesa-card-detail-meta flex items-center gap-1.5 text-sm font-semibold text-emerald-700">
                      {r.personas && <><Users className="h-4 w-4" />{r.personas}</> }
                      <Clock className="ml-1.5 h-4 w-4" />{r.hora}
                    </div>
                  </div>
                )}
                {m.status === "available" && (
                  proxima
                    ? <div className="mesa-card-detail mt-2 space-y-0.5">
                        ...
                        <div className={`mesa-card-detail-meta flex items-center gap-1.5 truncate text-sm font-semibold ${isTableBlockReservation(proxima) ? "text-rose-700" : "text-emerald-700"}`} title={agenda.map((a) => `${a.hora} ${a.nombre}`).join(" · ")}>
                          {isTableBlockReservation(proxima) ? "Bloqueada" : "Reservada"}
                          {!isTableBlockReservation(proxima) && proxima.personas && <><Users className="h-4 w-4" />{proxima.personas}</> }
                          <Clock className="ml-1.5 h-4 w-4" />{proxima.hora}{futuras.length > 1 ? ` +${futuras.length - 1}` : ""}
                        </div>
                      </div>
      ...
      <Modal open={!!sel && !cancelReservaId} onClose={() => { setSel(null); setEditing(false); setEditError(""); }}>
        {sel && (() => {
          ...
          <div className="space-y-4">
                  ...
                  <>
                    ...
                    {focusR.estado === "sentada" && (
                      <button onClick={() => void handleDesSentar(focusR)}
                        ...
                        ↩ Volver a confirmada (sin sentar)
                      </button>
                    )}
                  </>
                ) : (
            ...
            {/* Reservar la mesa para más tarde aunque ahora esté ocupada (walk-in / sentada) */}
            {!editing && sel.status === "occupied" && (
              <button onClick={() => { openNuevaParaMesa(sel); setSel(null); }}
                ...
                + Reservar T{sel.numero} para más tarde
              </button>
            )}
          ...
          wiOk ? (
            <div className="space-y-4 text-center">
              <div className="rounded-xl bg-emerald-700 p-6 text-white">
                <p className="text-5xl font-black">{mesaLabel(wiMesaIds)}</p>
                <p className="mt-2 font-bold text-emerald-100">{wiMesaIds.length > 1 ? "Mesas ocupadas" : "Mesa ocupada"} · Walk-In</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-gray-900">
                    {wiMesaIds.length ? `${mesaLabel(wiMesaIds)} — Walk-In` : "Walk-In"}
                  </h2>
                  <p className="text-sm text-gray-500">
                    {wiMesaIds.length
                      ? `${wiPlazas} plazas · toca otra mesa para añadirla al grupo`
                      : "Selecciona una o varias mesas y registra entrada directa"}
                  </p>
                </div>
              ...
              {wiMesaIds
                .map((id) => mesas.find((x) => x.id === id))
                .filter((mc): mc is MesaConEstado => !!mc)
                .map((mc) => {
                  const proxima = (mc.agenda ?? []).find((r) => !isTableBlockReservation(r) && r.estado !== "sentada" && r.estado !== "walkin");
                  return proxima ? (
                    <p key={mc.id} className="rounded-xl bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
                      ⚠️ T{mc.numero} tiene reserva a las {proxima.hora} ({proxima.nombre}). El walk-in la ocupará ahora.
                    </p>
                  ) : null;
                })}
              {wiShowPicker && (
                <div>
                  <label className="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Mesas *</label>
                  <div className="grid grid-cols-5 gap-1.5">
                    {mesasList.filter((m) => {
                      if (wiMesaIds.includes(m.id)) return true;
                      ...
                      return (
                      <button key={m.id}
                        onClick={() => setWiMesaIds((p) => p.includes(m.id)
                          ? p.filter((x) => x !== m.id)
                          : [...p, m.id].sort((a, b) => Number(a.slice(1)) - Number(b.slice(1))))}
                        className={`rounded-lg border-2 px-2 py-1.5 text-center transition-colors ${
                          wiMesaIds.includes(m.id)
              ...
              <button onClick={submitWalkIn}
                ...
                {wiMesaIds.length ? `Ocupar ${mesaLabel(wiMesaIds)} ahora` : "Ocupar mesa ahora"}
              </button>
            </div>
          )
        )}
