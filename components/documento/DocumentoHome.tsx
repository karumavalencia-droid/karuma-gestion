...
export function DocumentoHome() {
  ...
  const [alerts, setAlerts] = useState({ unpaidInvoices: 0, expiringContracts: 0, duplicateCandidates: 0, legalPending: 0, aiFailures: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(body);
      if (alertsResponse.ok) setAlerts(await alertsResponse.json() as typeof alerts);
      if (inboxResponse.ok) {
  ...
  async function importGmail() {
    if (!window.confirm("¿Importar adjuntos de Gmail? Solo se leerán mensajes con adjuntos y no se marcarán como leídos.")) return;
    setGmailBusy(true);
    setError("");
    setNotice("");
    try {
      const response = await fetch("/api/documentos/gmail/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ limit: 10 }) });
      const body = await response.json() as { imported?: number; skipped?: number; failed?: number; error?: string };
      if (!response.ok) throw new Error(body.error || "No se pudieron importar adjuntos");
      setNotice(`Gmail: ${body.imported || 0} importado(s), ${body.skipped || 0} omitido(s), ${body.failed || 0} con error.`);
      await load();
    } catch (requestError) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
          ...
          <Metric label="Facturas por pagar" value={alerts.unpaidInvoices} icon={<FolderOpen className="h-4 w-4" />} />
        ...
        <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
          {categories.map(([value, label]) => <button key={value} onClick={() => setCategory(category === value ? undefined : value)} className={`rounded-xl border px-3 py-3 text-left text-xs transition ${category === value ? "border-amber-400 bg-amber-500/15 text-amber-200" : "border-zinc-800 bg-zinc-950 text-zinc-400 hover:border-zinc-700 hover:text-zinc-100"}`}><span className="mb-2 block text-amber-400">{value === "idea" ? <Lightbulb className="h-4 w-4" /> : value === "image" ? <Camera className="h-4 w-4" /> : <Archive className="h-4 w-4" />}</span>{label}</button>)}
        </div>
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          ...
          <QuickAction icon={gmailBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />} label="Importar Gmail" onClick={() => void importGmail()} />
...
function Metric({ label, value, icon }: { label: string; value: string | number; icon: React.ReactNode }) { return <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4"><div className="mb-3 flex items-center gap-2 text-amber-400">{icon}<span className="text-[11px] uppercase tracking-wide text-zinc-500">{label}</span></div><p className="text-2xl font-semibold text-zinc-100">{value}</p></div>; }
