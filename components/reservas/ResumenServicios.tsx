...
function statsHechas(reservas: ReservaLocal[], servicio: ServicioLocal) {
  const hechas = reservas.filter((r) => r.servicio === servicio && ESTADOS_HECHOS.has(r.estado) && !isTableBlockReservation(r));
  return { mesas: hechas.length, pax: hechas.reduce((s, r) => s + (r.personas || 0), 0) };
}
  ...
  return (
    <div className="pointer-events-none fixed bottom-3 right-3 z-40 select-none rounded-xl bg-gray-900/90 px-3.5 py-2.5 text-xs text-white shadow-lg backdrop-blur">
      ...
      <div className="space-y-0.5 font-medium">
        ...
        <p className="whitespace-nowrap">Cena <span className="font-bold text-sky-300">{cena.mesas}</span> mesas · <span className="font-bold text-sky-300">{cena.pax}</span> pax</p>
      </div>
    </div>
  );
}
