    ...
    <section className="mb-5 rounded-2xl border border-amber-900/50 bg-[#0d0d0d] p-4 text-zinc-100 shadow-xl sm:mb-6 sm:p-5">
      ...
      {!loading && analytics ? <>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <Metric label="Personal" value={money(analytics.metrics.laborCost)} detail={analytics.metrics.laborCostRate == null ? "Nómina + Seguridad Social" : `${analytics.metrics.laborCostRate}% de las ventas`} status={analytics.metricStatus.laborCost} icon={<Users className="h-3.5 w-3.5 text-amber-400" />} />
          <Metric label="Costes fijos" value={money(analytics.metrics.fixedCosts)} detail="Alquiler, suministros, impuestos y otros" status={analytics.metricStatus.fixedCosts} icon={<Wallet className="h-3.5 w-3.5 text-amber-400" />} />
          <Metric label="Comisiones" value={money(analytics.metrics.platformCommissions)} detail={analytics.metrics.platformCommissionRate == null ? "Uber Eats, Glovo y otras" : `${analytics.metrics.platformCommissionRate}% de las ventas`} status={analytics.metricStatus.platformCommission} icon={<Wallet className="h-3.5 w-3.5 text-amber-400" />} />
          <Metric label={analytics.metrics.operatingProfit == null ? "Resultado parcial" : "Resultado operativo"} value={money(analytics.metrics.operatingProfit ?? analytics.metrics.operatingProfitPartial)} detail={analytics.metrics.operatingProfit == null ? "Solo descuenta los costes confirmados disponibles" : `${analytics.metrics.operatingProfitMargin}% de margen operativo`} status={analytics.metrics.operatingProfit == null ? analytics.metricStatus.partialOperatingProfit : analytics.metricStatus.operatingProfit} icon={<Wallet className="h-3.5 w-3.5 text-amber-400" />} />
